...
object HypixelUtils {
    ...
    fun getGEXP(): Boolean {
        ...
        val guildData =
            APIUtil.getJSONResponse("https://api.hypixel.net/guild?key=" + RysmConfig.apiKey + ";player=" + uuid)["guild"].asJsonObject
    ...
    fun getWinstreak(): Boolean {
        val uuid = mc.thePlayer.gameProfile.id.toString().replace("-", "")
        val playerStats =
            APIUtil.getJSONResponse("https://api.hypixel.net/player?key=${RysmConfig.apiKey};uuid=$uuid").asJsonObject["player"].asJsonObject["stats"]
