...
public abstract class InGameHudMixin {
    ...
    private void cancelScoreboard(MatrixStack matrices, ScoreboardObjective objective, CallbackInfo ci) {
        if (!WyvtilsConfig.INSTANCE.getSidebar()) ci.cancel();
        if (MinecraftClient.getInstance().currentScreen instanceof SidebarGui) ci.cancel();
        matrices.push();
        float iHaveNoIdeaWhatToNameThisFloat = WyvtilsConfig.INSTANCE.getSidebarScale() - 1.0f;
        matrices.translate(-WyvtilsConfig.INSTANCE.getSidebarX() * iHaveNoIdeaWhatToNameThisFloat, -WyvtilsConfig.INSTANCE.getSidebarY() * iHaveNoIdeaWhatToNameThisFloat, 0.0f);
    ...
    private int modifyY(int m) {
        if (WyvtilsConfig.INSTANCE.getSidebarPosition()) {
            return WyvtilsConfig.INSTANCE.getSidebarY();
        } else {
            return m;
        }
    }
    ...
    private int modifyX2(int m) {
        if (WyvtilsConfig.INSTANCE.getSidebarPosition()) {
            return WyvtilsConfig.INSTANCE.getSidebarX() - 1;
        } else {
            return m;
        }
    }
    ...
    private void removeBackground(MatrixStack matrices, int x1, int y1, int x2, int y2, int color) {
        if (WyvtilsConfig.INSTANCE.getSidebarBackground()) {
            DrawableHelper.fill(matrices, x1, y1, WyvtilsConfig.INSTANCE.getSidebarX() - 1, y2, WyvtilsConfig.INSTANCE.getSidebarBackgroundColor().getRGB());
        }
    }
